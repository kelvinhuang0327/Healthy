...
class HealthMetricCsvImportValidationError(HealthMetricCsvImportError):
    ...
    def __str__(self) -> str:  # pragma: no cover - compatibility for generic handlers
...
def _validate_exact_ascii_utf8(raw: bytes) -> str:
    try:
...
def _build_row_fingerprint(row: ParsedHealthMetricRow) -> str:
    canonical_payload = {
...
def parse_health_metric_rows(payload: bytes) -> list[ParsedHealthMetricRow]:
    ...
    text = _validate_exact_ascii_utf8(payload)
    ...
    supported_headers = {MANDATORY_HEADER, *OPTIONAL_METRIC_HEADERS}
    unknown = [header for header in headers if header not in supported_headers]
    if unknown:
        _validation_error(row_number=None, field="header", code="UNKNOWN_HEADER")

    optional_count = sum(1 for header in headers if header in OPTIONAL_METRIC_HEADERS)
    if optional_count == 0:
    ...
    for row_number, raw_row in enumerate(reader, start=1):
        ...
        # Ensure missing required keys from csv.DictReader are treated as blanks.
        ...
        systolic = _parse_int(
            normalized_row["systolic_bp_mm_hg"],
            row_number=row_number,
        ...
        diastolic = _parse_int(
            normalized_row["diastolic_bp_mm_hg"],
            row_number=row_number,
        ...
        note = normalized_row["note"] or None
        ...
        fingerprint = _build_row_fingerprint(row)
        row = ParsedHealthMetricRow(
